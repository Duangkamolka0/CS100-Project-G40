...
function validateLocation() {
  ...
  if (!locationPattern.test(locationInput.value)) {
    errorElement.textContent = "Please enter the location in the format 'place,province'.";
    return false;
  } else {
    errorElement.textContent = ""; // Clear the error message when valid
  }
...
async function submitForm(event) {
  ...
  if (!validateName() || !validateStudentID() || !validateEmail() || validateLocation()) {
    return;
  }
...
function displayValues(data) {
  ...
  const formContainer = document.querySelector('.form-container');
    ...
    alert('We have received your acticity already!');
  });
